...
- Added a bundled `gode_node` helper and npm snapshot materialization for package paths and forked dependency trees, so `child_process.fork()`, ESM `import.meta.url` path lookups, and native `.node` loading can run real desktop package probes without relying on a user-installed Node executable at runtime.
- Added packaged-plugin export smoke coverage for Windows, Linux, and macOS using a minimal Godot `node-llama-cpp` project.
- Hardened release packaging by retrying transient TypeScript compiler download failures.
