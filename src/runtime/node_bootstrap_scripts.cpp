...
std::string esm_bootstrap_script() {
	return "(function() {"
		   ...
		   "  }"
		   "  if (normalized.includes('/.gode/typescript/')) {"
		   "    const markerIndex = normalized.indexOf(cacheMarker);"
		   "    if (markerIndex >= 0) return 'res://' + normalized.slice(markerIndex + cacheMarker.length);"
		   "  }"
